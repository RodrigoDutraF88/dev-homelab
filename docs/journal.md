...
Compose reads .env values through variable interpolation, so a literal dollar sign has to be written as $$. This matters for the bcrypt hash from htpasswd, which is full of them. Written with single dollars, Compose treats $2y and $05 as variable references, substitutes them with empty strings, and stores a truncated hash, which silently locks you out of the dashboard rather than throwing an error.
...
Scheduling: a systemd timer rather than cron, because cron only fires if the machine happens to be awake at that minute and silently skips otherwise, which on a laptop is how backups quietly went days stale.
